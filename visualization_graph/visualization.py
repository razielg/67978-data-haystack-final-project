...
def get_partition_heads_node_id(graph, party_leaders = None):
    """
    ...
    for node in graph.nodes:
        ...
        if node_data['name'] in party_leaders:
...
def plot_graph_with_color(G: nx.Graph, communities,knesset_num, party_leaders =None):
    partition_heads_with_node_id = get_partition_heads_node_id(G, party_leaders)
    ...
    plt.title(f"Detected Communities (Color) VS Coalition/Opposition (Shape) For knesset {knesset_num}:", fontsize=30)
    plt.tight_layout()
